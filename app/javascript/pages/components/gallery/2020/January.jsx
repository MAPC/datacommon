...
class January extends React.Component {
  componentDidMount() {
    Promise.all([
      ...
      let svg = d3.select('.calendar-viz')
        .append('svg')
      ...
      function ticker(svg) {
        const now = d3.select('.calendar-viz__subtitle')
          .append('div')
          .attr('class', 'calendar-viz__year')
          .text(formatDate(allKeyframes[0][0]));

        d3.select('.calendar-viz__subtitle')
          .append('div')
          .attr('class', 'calendar-viz__subtitle-text')
          .text('NAICS Average Monthly Employment for MAPC Region');

        d3.select('.calendar-viz__subtitle')
          .append('div')
          .attr('class', 'calendar-viz__download-link')
  ...
  render() {
    return (
      <>
        <h1 className="calendar-viz__title">Employment by Industry</h1>
        <div className="calendar-viz container" />
        <div className="calendar-viz__subtitle" />
